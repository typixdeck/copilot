...
**选固件项目 → 选版本 → 下载/本地导入 → 确认切换 → 写入并验证 → 后续可切回已缓存版本。**
...
| 页面 | 用户能做什么 |
| --- | --- |
| 固件商店 | 按项目浏览官方 / 自研 / 第三方固件；搜索、选版本、收藏和下载到本机 |
...
全屏布局适合约 800×600 逻辑分辨率；Tab / 方向键 / Enter 可完成选择，Esc 返回。危险提交使用独立确认页，不能由列表中的单次 Enter 直接触发。未写入阶段可以取消，正在写 Flash 时必须按工具支持的安全边界处理；界面不提供强杀式“取消”。
...
普通用户可以导入和检查未知来源文件，但未建立信任、未确认板级兼容的条目不能进入标准刷写流程。本地开发模式只在明确审阅具体包后形成单次授权，不能永久关闭校验。来自固件包的 shell 脚本、hook 或任意命令一律不作为执行步骤。
